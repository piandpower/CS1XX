...
blocks = [block_1, block_3, block_2]
    
def amp_diff(SB1, SB2):
    return abs( SB1.average_amplitude - SB2.average_amplitude )

# lambda SB1, SB2: abs(SB1.average_amplitude - SB2.average_amplitude)


#  diff_list= []
#  for blok in sorted_lsit...
#              amp_diff(target_sample_block, blok)
#              diff_list += [amp_diff]
#  return diff_list
...
def find_closest_sample_block_index(target_sample_block, sorted_list_of_sample_blocks):
    ...
    # The active region is end - beginning positions long.
    while beginning < end:
        middle = (end + beginning)/2 # integer division! 
